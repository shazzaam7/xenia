...
// store). Only consulted when the build embedded no bundle: next to the
// executable (installed layout), then the repo assets dir. The assets lookup
// is cwd-independent: it checks the process working directory (dev runs from
// the repo root) and walks up from the executable folder. Empty when none
// exists.
std::string CaBundlePath() {
  static const std::string cached = []() -> std::string {
    std::error_code ec;
    const auto exe_folder = xe::filesystem::GetExecutableFolder();
    if (std::filesystem::exists(exe_folder / "cacert.pem", ec)) {
      return xe::path_to_utf8(exe_folder / "cacert.pem");
    }
    // Dev fallback when the embed step had no bundle: <exe>/../../assets/
    // covers build/bin/<Platform>/<Config>/ layouts regardless of cwd.
    for (auto dir = exe_folder; !dir.empty(); dir = dir.parent_path()) {
      const auto candidate = dir / "assets" / "cacert.pem";
      std::error_code probe_ec;
      if (std::filesystem::exists(candidate, probe_ec)) {
        return xe::path_to_utf8(candidate);
      }
      // Don't climb past the filesystem root (parent == self).
      if (dir.parent_path() == dir) {
        break;
      }
    }
    const auto dev_bundle = std::filesystem::path("assets") / "cacert.pem";
    if (std::filesystem::exists(dev_bundle, ec)) {
      return xe::path_to_utf8(dev_bundle);
    }
    return {};
  }();
  return cached;
...
// Build-embedded CA bundle (generated by cmake/EmbedCaBundle.cmake from
// assets/cacert.pem). Empty when the bundle was absent at build time.
extern const char kEmbeddedCaBundle[];
extern const unsigned long kEmbeddedCaBundleSize;
...
bool HttpGet(const std::string& url, std::string* body) {
  ...
  // Trust source, in order: the build-embedded bundle (authoritative — an
  // external file cannot shadow it), then a file next to the executable,
  // then assets/ for dev runs. Schannel ignores all of this; wolfSSL needs
  // one of them (no OS trust store).
  if (kEmbeddedCaBundleSize > 0) {
    curl_blob blob;
    blob.data = const_cast<char*>(kEmbeddedCaBundle);
    blob.len = kEmbeddedCaBundleSize;
    blob.flags = CURL_BLOB_NOCOPY;  // static storage, valid for process life
    curl_easy_setopt(curl, CURLOPT_CAINFO_BLOB, &blob);
  } else {
    const std::string ca_bundle = CaBundlePath();
    if (!ca_bundle.empty()) {
